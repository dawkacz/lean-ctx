...
(function () {
  ...
  var TIPS = {
    ...
    total_tokens_saved: 'Lifetime total of tokens saved through all compression modes since installation. An *estimate*: search/grep savings assume a native grep would have cost 2.5x the raw matched output, and cache hits count the full original as saved. The ROI view shows the stricter, measured-only ledger.',
    cost_saved: 'Estimated dollar savings based on average LLM token pricing ($2.50/1M input tokens) applied to the estimated tokens-saved total above (which includes the 2.5x search baseline assumption).',
